...
export default class WebGLFunctions {
    ...
    public static updateWebglFiles(req: Request, res: Response, next: NextFunction) {
        const fileNames: string[] = [];
        const files: Promise<Result<File>>[] = [];
        const busboy = Busboy({headers: req.headers});
        ...
        busboy.on('file', (fieldname: string, file: NodeJS.ReadableStream, info: FileInfo) => {
            const {filename, encoding, mimeType} = info;
            files.push(fileRepo.updateFile(req.params.fileID, req.params.containerID, req.currentUser!, filename, file as Readable));
            fileNames.push(filename);
        });
    ...
    private static tagWebGL(tag: Tag, file: File) {
        void tagRepo.tagFile(tag, file);
    }

    public static listWebglFilesAndTags(req: Request, res: Response, next: NextFunction) {
        if (req.container) {
            tagRepo
                .listWebglFilesAndTags(req.container.id!)
                .then((result) => {
