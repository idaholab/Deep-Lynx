...
export default class ContainerRoutes {
    ...
    private static archiveContainer(req: Request, res: Response, next: NextFunction) {
        ...
        if (req.query.permanent?.toString().toLowerCase() === 'true') {
            repository
    ...
    private static importContainer(req: Request, res: Response, next: NextFunction) {
        ...
        busboy.on('finish', () => {
            ...
            if (input.data_versioning_enabled) input.data_versioning_enabled = input.data_versioning_enabled?.toString().toLowerCase() === 'true';

            containerImport
                .ImportOntology(user, input as ContainerImportT, fileBuffer, req.query.dryrun?.toString().toLowerCase() === 'true', false, '')
                .then((result) => {
                    result.asResponse(res);
                })
                .catch((err) => res.status(500).send(err))
