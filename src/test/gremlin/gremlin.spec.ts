...
import GremlinAdapter from "../../data_access_layer/mappers/adapters/gremlin/gremlin"
...
describe('A Gremlin Adapter', async () => {
    ...
    it('can add a simple vertex', (done) => {
        g!.vertices.add('person', { "name": faker.name.findName() })
            .then((resp) => {
    ...
    it('can retrieve a vertex', async () => {
        const vertex = await g!.vertices.add('person', { "name": faker.name.findName(), "hobbies": ["baking"] });
        g!.vertices.retrieve(vertex.value.id)
            .then((resp) => {
                expect(resp.isError).false;
            })
    });
    ...
    it('can update a simple vertex', async () => {
        try {
            ...
            let updated = await g!.vertices.update(vertex.value.id, { "name": secondName });
    ...
    it('can permanently delete a vertex', async () => {
        try {
            let vertex = await g!.vertices.add('person', { "name": faker.name.findName(), "hobbies": ["baking"] });
    ...
    /* Currently edge retrevial fails due to a bad ID lookup. This is very specific to the instance of gremlin. This should be corrected at some point, but we don't need it for pure export functionality and so it has been ignored on purpose
