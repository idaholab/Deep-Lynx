...
describe('A Timeseries Entry', async () => {
    ...
    it('can be saved to the table', async () => {
        ...
        const saved = await TimeseriesEntryMapper.Instance.BulkCreate([entry]);
    ...
    it('can be bulk saved to the proper tables', async () => {
        ...
        const saved = await TimeseriesEntryMapper.Instance.BulkCreate([entry, entry2]);
        expect(saved.isError, saved.error?.error).false;
