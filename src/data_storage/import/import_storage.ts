...
export default class ImportStorage extends PostgresStorage {
    ...
    public async ListAndSort(importAdapterID:string, offset:number, limit:number, sortBy: string, sortDesc: boolean): Promise<Result<ImportT[]>>{
        if(sortDesc){
            return super.rows<ImportT>(ImportStorage.listSortStatementDesc(importAdapterID, offset, limit, sortBy))
        }

        return super.rows<ImportT>(ImportStorage.listSortStatementAsc(importAdapterID,offset,limit, sortBy))
    }
    ...
    private static listSortStatementAsc(importAdapterID: string, offset:number, limit:number, sortBy: string): QueryConfig {
        return {
    ...
    private static listSortStatementDesc(importAdapterID: string, offset:number, limit:number, sortBy: string): QueryConfig {
        return {
            text: `SELECT imports.id,
                    imports.data_source_id,
                    imports.status,
