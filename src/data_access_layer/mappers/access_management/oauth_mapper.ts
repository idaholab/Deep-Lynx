...
/*
    OAuthMapper extends the Postgres database Mapper class and allows
...
export default class OAuthMapper extends Mapper {
    ...
    private fullUpdateStatement(userID: string, ...applications: OAuthApplication[]): string {
        ...
        const values = applications.map((oauth) => [oauth.id, oauth.name, oauth.owner_id, oauth.description, userID]);
