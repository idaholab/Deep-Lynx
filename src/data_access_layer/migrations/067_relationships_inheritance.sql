...
CREATE OR REPLACE FUNCTION get_metatype_keys(arg_metatype_id bigint, arg_container_id bigint) RETURNS setof record
AS $$
WITH RECURSIVE parents AS (
...
) SELECT metatype_keys.*, p.name AS metatype_name FROM parents p INNER JOIN metatype_keys ON metatype_keys.metatype_id = p.id;
$$ LANGUAGE sql;
