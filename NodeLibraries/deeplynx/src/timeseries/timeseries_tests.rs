...
mod main_tests {
    ...
    use crate::timeseries::repository::{BucketColumn, BucketRepository, ChangeBucketPayload};
    ...
    async fn all_types_bucket_insertion() -> Result<(), TestError> {
        let mut config = Configuration::new(None).unwrap();
        ...
        for column in columns {
            ...
            let data_type = column
                .udt_name
                .ok_or(TimeseriesError::Unwrap("udt type unwrap".to_string()))?;
       ...
       /*  TODO: reenable once we correct the download stream functionality
    ...
    async fn bucket_update_insertion() -> Result<(), TestError> {
        ...
        let mut column = bucket
            ...
            .cloned()
            .ok_or(TimeseriesError::Unwrap("column finding unwrap".to_string()))?;
