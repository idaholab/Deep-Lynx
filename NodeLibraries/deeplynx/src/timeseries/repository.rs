...
impl BucketRepository {
    ...
    pub async fn create_bucket(&self, payload: ChangeBucketPayload) -> Result<Bucket, TimeseriesError> {
        ...
        for column in &columns {
            ...
            if !column_map.contains_key(&table_name) {
                ...
                let mut internal = column_map
                    .get(&table_name)
                    .ok_or(TimeseriesError::Unwrap("table hash map fetch unwrap".to_string()))?
                    .clone();
    ...
    pub fn infer_legacy_schema<T: Read>(reader: T) -> Result<Vec<LegacyTimeseriesColumn>, TimeseriesError> {
        let mut csv_reader = csv::ReaderBuilder::new().from_reader(reader);
 ...
 /// `download_data_simple` is a simple data download function to bring this microservice into
        ...
        tokio::task::spawn(async move {
            ...
            match result {
                ...
                Err(e) => match status_tx.send(StreamStatusMessage::Error(e)).await {
                    Ok(_) => Ok(()),
                    Err(e) => Err(TimeseriesError::Thread(format!("ingest csv problem: {}", e))),
                },
            }
        });
    ...
    pub fn read_data(&mut self, bytes: Vec<u8>) -> Result<(), TimeseriesError> {
        let channel = self
            .reader_status_channel
            .as_mut()
            .ok_or(TimeseriesError::Unwrap("no stream status channel".to_string()))?;
        ...
        match channel.try_recv() {
            ...
            Err(e) => match e {
                ...
                tokio::sync::mpsc::error::TryRecvError::Disconnected => {
                    return Err(TimeseriesError::Thread("status thread disconnected".to_string()))
                }
            },
        }
