  ...
  <v-dialog v-model="dialog" @click:outside="reset()" max-width="70%">
    <template v-slot:activator="{on}">
      <v-icon
        ...
        v-on="on"
      >mdi-delete</v-icon>
      <v-btn v-if="!icon" color="primary" dark class="mt-2" v-on="on">{{$t("events.deleteAction")}}</v-btn>
    </template>

    <v-card class="pt-1 pb-3 px-2">
      <v-card-title>
        <span class="headline text-h3">{{$t('events.deleteAction')}}</span>
      </v-card-title>
      <v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn color="error" text @click="deleteEventAction()" >{{$t("general.delete")}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
