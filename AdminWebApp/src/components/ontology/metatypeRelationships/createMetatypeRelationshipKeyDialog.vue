  ...
  <v-dialog v-model="dialog" @click:outside="dialog = false" max-width="60%">
    <template v-slot:activator="{ on }">
      <v-icon
          ...
          v-on="on"
      >mdi-pencil</v-icon>
      <v-btn v-if="!icon" color="primary" dark class="mt-2" v-on="on">{{$t("createMetatypeRelationshipKey.newKey")}}</v-btn>
    </template>

    <v-card class="pt-1 pb-3 px-2">
      <v-card-title>
        <span class="headline text-h3">{{$t('createMetatypeRelationshipKey.newKey')}}</span>
      </v-card-title>   
      <v-card-text>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" text @click="dialog = false" >{{$t("createMetatypeRelationshipKey.cancel")}}</v-btn>
        <v-btn color="primary" :disabled="!formValid" text @click="createMetatypeKey()">{{$t("createMetatypeRelationshipKey.create")}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
