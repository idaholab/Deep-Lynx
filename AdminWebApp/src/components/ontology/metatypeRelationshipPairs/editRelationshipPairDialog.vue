  ...
  <v-dialog v-model="dialog" max-width="60%">
    <template v-slot:activator="{ on }">
      <v-icon
          ...
          v-on="on"
      >mdi-pencil</v-icon>
      <v-btn v-if="!icon" color="primary" dark class="mt-2" v-on="on" style="margin-top: 0px !important;">{{$t("editMetatypeRelationshipPair.editMetatypeRelationshipPair")}}</v-btn>
    </template>
    ...
    <v-card class="pt-1 pb-3 px-2">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" text @click="dialog = false" >{{$t("editMetatypeRelationshipPair.cancel")}}</v-btn>
        <v-btn color="primary" text :disabled="!valid" @click="editRelationshipPair()">{{$t("editMetatypeRelationshipPair.save")}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
