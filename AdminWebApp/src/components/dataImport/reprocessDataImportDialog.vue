  ...
  <v-dialog v-model="dialog" @click:outside="reset()"  max-width="60%">
    <template v-slot:activator="{ on: dialog }">
      <v-tooltip bottom v-if="icon">
       ...
       <span>{{$t('reprocessDataImport.tooltip')}}</span>
      </v-tooltip>

      <v-btn v-if="!displayIcon" color="primary" dark class="mb-1" v-on="on">{{$t("reprocessDataImport.reprocessImport")}}</v-btn>
    </template>

    <v-card class="pt-1 pb-3 px-2">
      <v-card-title>
        <span class="headline text-h3">{{$t('reprocessDataImport.title')}}</span>
      </v-card-title>   
      <v-card-text>
      ...
      <v-card-actions>
        <div v-if="!loading">
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="reset()">{{$t("reprocessDataImport.cancel")}}</v-btn>
...
@Component
export default class ReprocessDataImportDialog extends Vue {
  @Prop({required: true})
  containerID!: string
