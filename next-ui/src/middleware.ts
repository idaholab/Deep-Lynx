...
import Authentication from "@/app/_lib/authentication_service";
const auth = new Authentication();

// This function can be marked `async` if using `await` inside
export function middleware(request: NextRequest) {
    const publicPages = ['/', '/login', '/reset-password', '/validate', '/register', '/container-invite'];
    const authRequired = !publicPages.includes(request.nextUrl.pathname);
    ...
    if (request.url === '/' && request.headers.get('token')) {
        auth.LoginFromToken(request.headers.get('token') as string, request.headers.get('state') as string)
            .then((result) => {
                if (result) {
