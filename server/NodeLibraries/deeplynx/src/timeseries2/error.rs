...
pub enum TSError {
    ...
    #[error("clap:: {0}")]
    FromClapError(clap::error::Error),

    #[error("{0}")]
    DataFusionError(#[from] DataFusionError),

    #[error("amiquip:: {0}")]
    AmiquipError(#[from] amiquip::Error),
...
impl TSError {
    pub fn to_response(&self, request: Request, msg: String) -> Response {
        let error_string = format!("{:?}", &self);
        // Response::from_tuple("".to_string(), true, error_string)
        Response {
