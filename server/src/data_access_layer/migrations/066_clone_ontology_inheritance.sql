...
BEGIN
    ...
    INSERT INTO metatypes_inheritance(
        parent_id,
        child_id)
    SELECT  p.id as parent_id,
            c.id as child_id
    FROM metatypes_inheritance i
             LEFT JOIN metatypes p ON p.old_id = i.parent_id
             LEFT JOIN metatypes c ON c.old_id = i.child_id
    WHERE p.ontology_version = targetOntology AND c.ontology_version = targetOntology AND p.container_id = containerID AND c.container_id = containerID;

    REFRESH MATERIALIZED VIEW metatype_full_keys;
