...
export default class DeleteDataSourceDialog extends Vue {
  ...
  deleteSource() {
    ...
    this.$client.deleteDataSources(this.containerID, this.dataSource!.id, {forceDelete: true})
    .then(() => {
      this.reset()
  ...
  archiveSource() {
    this.deleteLoading = true
    this.$client.deleteDataSources(this.containerID, this.dataSource!.id, {forceDelete: false, archive: true})
        .then(() => {
          this.reset()
