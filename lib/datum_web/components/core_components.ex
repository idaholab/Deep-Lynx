defmodule DatumWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "search"} = assigns) do
    ...
    <input
      ...
      class={[
        "peer relative col-start-1 row-start-1 border-0 bg-transparent py-1.5 text-white placeholder:text-gray-400 focus:ring-0 sm:text-sm/6",
        @errors == [] && "border-zinc-300 focus:border-zinc-400",
        @errors != [] && "border-rose-400 focus:border-rose-400"
      ]}
      {@rest}
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        ...
        <p :if={@subtitle != []} class="mt-2 text-sm leading-6 text-white-700">
  ...
  @doc ~S"""
  Renders a file-explorer like table
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DatumWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DatumWeb.Gettext, "errors", msg, opts)
    end
  end
